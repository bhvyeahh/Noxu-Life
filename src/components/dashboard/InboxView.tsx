...
export function InboxView({
  ...
  const showReviewBanner = (() => {
    if (
      !activeChat?.expiresIn ||
      activeChat.expiresIn === "Active Now" ||
      activeChat.expiresIn === "Hold"
    )
      return false;
  ...
  useEffect(() => {
    ...
    const actualChatId = openedChatId.startsWith("c-")
      ? openedChatId.slice(2)
      : openedChatId;
      ...
      {!openedChatId ? (
        <div className="px-4 sm:px-6 pb-6">
          <div className="relative flex bg-[#111111]/80 backdrop-blur-2xl border border-white/10 rounded-full p-1.5 mb-8 shadow-inner supports-[backdrop-filter]:bg-black/40">
            ...
            >
              ...
              {requests.length > 0 && (
                <span
                  className={`px-2 py-0.5 rounded-full text-[10px] font-bold transition-all ${inboxView === "requests" ? "bg-[#CF5C36] text-white shadow-[0_0_10px_rgba(207,92,54,0.5)]" : "bg-white/10 text-[#7C7C7C]"}`}
                >
                  {requests.length}
                </span>
              )}
            ...
            <div className="flex flex-col gap-5">
              {requests.length === 0 && (
                <div className="flex flex-col items-center justify-center py-24 px-4 animate-in fade-in zoom-in-95 duration-500">
                  ...
                  <p className="text-[#7C7C7C] text-sm text-center max-w-xs leading-relaxed">
                    You have no pending requests. Broadcast a new vibe to get
                    the party started.
                  </p>
                </div>
              )}
              ...
              {requests.map((req) => (
                <div
                  key={req.id}
                  className="p-6 rounded-[32px] bg-gradient-to-b from-white/[0.03] to-transparent border border-white/5 hover:border-white/10 transition-all duration-300 shadow-lg hover:shadow-2xl hover:shadow-[#CF5C36]/5"
                >
                  <div className="flex items-start justify-between mb-5">
                    <div className="flex items-center gap-4">
                      <div className="relative">
                        <img
                          src={req.user.avatar}
                          onClick={() => setViewingProfile(req.user)}
                          className="w-14 h-14 rounded-full object-cover border-2 border-[#111] shadow-lg cursor-pointer hover:opacity-80 transition-opacity"
                        />
                      </div>
                      <div>
                        ...
                        <p className="text-xs text-[#7C7C7C] mt-0.5 flex items-center gap-1.5">
                          <Sparkles className="w-3 h-3 text-[#CF5C36]" />{" "}
                          {req.beaconTitle}
                        </p>
                      </div>
                    </div>
                    <span className="text-[10px] text-[#7C7C7C] font-semibold uppercase tracking-widest">
                      {req.timeAgo}
                    </span>
                  </div>
                  ...
                  <div className="p-4 bg-black/40 rounded-2xl border border-white/5 mb-6 relative shadow-inner">
                    ...
                    <p className="text-sm text-white/90 font-light leading-relaxed italic relative z-10">
                      "{req.message}"
                    </p>
                  </div>
                  ...
                  {req.status === "pending" ? (
                    <div className="grid grid-cols-3 gap-3">
                      <button
                        ...
                        className="group flex items-center justify-center gap-1.5 h-12 rounded-2xl bg-red-500/10 text-red-400 hover:bg-red-500 hover:text-white text-xs font-semibold transition-all duration-300"
                      >
                      ...
                      <button
                        ...
                        className="group flex items-center justify-center gap-1.5 h-12 rounded-2xl bg-white/5 text-white/70 border border-white/10 hover:bg-white/10 hover:text-white text-xs font-semibold transition-all duration-300"
                      >
                      ...
                      <button
                        ...
                        className="group flex items-center justify-center gap-1.5 h-12 rounded-2xl bg-gradient-to-b from-[#CF5C36] to-[#b04a29] text-white hover:from-[#E36940] hover:to-[#CF5C36] shadow-[0_4px_15px_rgba(207,92,54,0.3)] text-xs font-semibold transition-all duration-300"
                      >
                    ...
                    <div className="flex items-center justify-between p-2 pl-5 rounded-2xl bg-white/5 border border-white/10">
                      ...
                      <div className="flex gap-2">
                        <button
                          onClick={() => {
                            setInboxView("active");
                            setOpenedChatId(`c-${req.id}`);
                          }}
                          className="px-5 h-10 rounded-xl bg-white/10 hover:bg-white/15 text-white text-xs font-semibold transition-all"
                        >
                          Chat
                        </button>
                        <button
                          onClick={() => handleAccept(req.id)}
                          className="px-5 h-10 rounded-xl bg-[#CF5C36] text-white hover:bg-[#b04a29] text-xs font-semibold transition-all shadow-md"
                        >
                          Accept
                        </button>
                      </div>
          ...
          {inboxView === "active" && (
            <div className="flex flex-col gap-3">
              {activeChats.length === 0 && (
                <div className="text-center py-20 text-[#7C7C7C] text-sm italic">
                  No active chats.
                </div>
              )}
              {activeChats.map((chat) => (
                ...
                >
                  <img
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      setViewingProfile(chat.user);
                    }}
                    className="w-16 h-16 rounded-full object-cover border border-white/10 shadow-md group-hover:border-white/20 transition-colors cursor-pointer hover:opacity-80"
                  />
                  <div className="flex-1 overflow-hidden pr-2">
                    <div className="flex justify-between items-center mb-1.5">
                      ...
                      <span className="text-[11px] font-mono text-[#CF5C36] flex items-center gap-1 bg-[#CF5C36]/10 px-2 py-0.5 rounded-full border border-[#CF5C36]/20">
                        ...
                        {formatTimeLeft(chat.expiresIn)}
                      </span>
                    </div>
                    <p className="text-sm text-[#7C7C7C] truncate font-light tracking-wide">
                      {chat.beaconTitle}
                    </p>
                  </div>
                </div>
        ...
        <div className="fixed inset-0 z-50 bg-[#000000] flex flex-col animate-in slide-in-from-right-8 duration-300">
            ...
            <div className="flex items-center justify-between px-2 py-3">
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="p-2 rounded-full hover:bg-white/10 transition-all text-white/80 hover:text-white"
                >
                ...
                <div className="flex items-center gap-3">
                  <img
                    ...
                    onClick={() => setViewingProfile(activeChat?.user)}
                    className="w-10 h-10 rounded-full object-cover border border-white/10 shadow-sm cursor-pointer hover:opacity-80 transition-opacity"
                  />
                  <div>
                    <h3 className="font-semibold text-white leading-tight">
                      {activeChat?.user.name}
                    </h3>
                    <p className="text-xs text-[#7C7C7C] truncate w-40 sm:w-auto font-medium">
                      {activeChat?.beaconTitle}
                    </p>
                  </div>
                </div>
              </div>
              <div className="flex items-center gap-3 pr-4">
                <div className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 bg-[#CF5C36]/10 border border-[#CF5C36]/20 text-[#CF5C36] rounded-full text-xs font-mono font-semibold shadow-inner">
                  ...
                  {formatTimeLeft(activeChat?.expiresIn)}
                </div>
          ...
          {showReviewBanner && (
            <div className="w-full bg-gradient-to-r from-[#CF5C36]/20 via-[#CF5C36]/5 to-transparent border-b border-[#CF5C36]/20 p-3 flex items-center justify-between px-4 z-10 backdrop-blur-md animate-in slide-in-from-top-2 duration-500">
              <div className="flex items-center gap-2.5">
                ...
                <span className="text-xs font-medium text-white/90 tracking-wide">
                  The event has ended. Rate the vibe.
                </span>
              </div>
              <button
          ...
          <div className="sm:hidden flex items-center justify-center gap-1.5 py-1.5 bg-[#CF5C36]/10 border-b border-[#CF5C36]/20 text-[#CF5C36] text-[10px] font-mono font-bold tracking-widest uppercase">
            <Timer className="w-3.5 h-3.5" /> Self-destructs in{" "}
            {formatTimeLeft(activeChat?.expiresIn)}
          </div>

          <div
            ref={scrollRef}
            className="flex-1 overflow-y-auto p-4 space-y-6 bg-gradient-to-b from-transparent to-[#111111]/30"
          >
            ...
            {liveMessages.map((msg: any) => (
              ...
              >
                ...
                <span className="text-[10px] font-medium text-[#7C7C7C] mt-1.5 px-2">
                  {msg.timestamp}
                </span>
              </div>
            ))}
